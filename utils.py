...
def shared_float32(x, name=""):
	return tf.Variable(tf.cast(np.asarray(x, dttype=np.float32), tf.float32), name=name)
...
def update_tensor(V, dim2, val):
	...
	Z = tf.scan(body, elems=(V, dim2, val), initalizer=tf.constant(1, shape=V.get_shape().as_list()[1:], dtype=tf.float32), name="Scan_Update")
	return Z
