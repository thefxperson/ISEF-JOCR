...
def MANN(input_var, target, batch_size=16, num_classes=5, memory_shape=(128,40), controller_size=200, input_size=20*20, num_reads=4):
    ...
    with tf.variable_scope("weights"):
        ...
        bias_sigma = tf.get_variable("bias_sigma", shape=shape, initializer=tf.constant_initializer(0))
        ...
        weight_readhidden = tf.get_variable("weight_read", shape=shape, initializer=tf.random_uniform_initializer(-1*high, high))
    ...
    def step(input_time1, input_time):  #network
        ...
        weight_write_time = update_tensor(weight_write_time, tf.reshape(weight_leastused_time1[-1]), 1.0 - tf.reshape(sigma_time, shape=[-1]))
        ...
        with tf.variable_scope("memory_time"):
            ...
            memory_time = tf.update_tensor(memory_time1, weight_leastused_time1[:,0], tf.constant(0., shape=[batch_size, memory_shape[1]]))
