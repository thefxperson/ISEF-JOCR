...
class OmniglotDataLoader:
    ...
    def fetch_batch(self, n_classes, batch_size, seq_length,
        ...
        if label_type == 'one_hot':
            ...
            label_dict = [[[int(j) for j in list(baseN(i, 5)) + [0] * (6 - len(baseN(i, 5)))]
                      for i in np.random.choice(range(5 ** 6), replace=False, size=n_classes)]
            ...
            #print("sq enc", seq_encoded.shape)
            seq_encoded_shifted = np.concatenate(
                [np.zeros(shape=[batch_size, 1, 30]), seq_encoded[:, :-1, :]], axis=1
            )
